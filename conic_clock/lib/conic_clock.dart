...
//  Color darkColor = Color(0xffFF007A);
  ...
  void _updateModel() {
    setState(() {
      ...
      _weather =
          '${widget.model.temperatureString} (${widget.model.low}-${widget.model.high})' +
              '\n' +
    ...
    setState(() {
      _now = !_lightYearMode
          ...
          : DateTime.fromMillisecondsSinceEpoch(
              (DateTime.now().millisecondsSinceEpoch * 4000).toInt());
      double dayProgress = (_now.hour * 60 * 60 * 1000 +
              _now.minute * 60 * 1000 +
              _now.second * 1000 +
              _now.millisecond) /
          (24 * 60 * 60 * 1000);
      ...
      _weatherColorOffsetH =
          lerpDouble(_weatherColorOffsetH, _weatherColorOffsetTargetH, 0.001);
      _weatherColorOffsetS =
          lerpDouble(_weatherColorOffsetS, _weatherColorOffsetTargetS, 0.001);
      _weatherColorOffsetV =
          lerpDouble(_weatherColorOffsetV, _weatherColorOffsetTargetV, 0.001);

      _tempColorOffsetS =
          lerpDouble(_tempColorOffsetS, _tempColorOffsetTargetS, 0.001);

      double lightColorH = _checkColorH(
          (cos(dayProgress * radians(360) + _weatherColorOffsetH)) * 360.0);
      double lightColorS = _checkColorS(0.15 +
          (sin(dayProgress * radians(360)) +
                  _weatherColorOffsetS +
                  _tempColorOffsetS) *
              0.1);
      double lightColorV = _checkColorV(
          0.7 + 0.3 * (sin(dayProgress * radians(180)) + _weatherColorOffsetV));

      double midColorH = _checkColorH(
          (sin(dayProgress * radians(1080)) + _weatherColorOffsetH) * 30.0 +
              230);
      double midColorS = _checkColorS(0.7 +
          0.2 *
              (sin(dayProgress * radians(720)) +
                  _weatherColorOffsetS +
                  _tempColorOffsetS));
      double midColorV = _checkColorV(
          0.7 + 0.2 * (sin(dayProgress * radians(180)) + _weatherColorOffsetV));

      double darkColorH = _checkColorH(
          ((cos(dayProgress * radians(360)) * 0.5 + 0.2) % 1.0 +
                  _weatherColorOffsetH) *
              360.0);
      double darkColorS =
          _checkColorS(0.8 + _weatherColorOffsetS + _tempColorOffsetS);
      double darkColorV = _checkColorV(0.25 +
          0.2 * (sin(dayProgress * radians(720)) + _weatherColorOffsetV));

      _lightColor =
          HSVColor.fromAHSV(1.0, lightColorH, lightColorS, lightColorV)
              .toColor();
      _midColor =
          HSVColor.fromAHSV(1.0, midColorH, midColorS, midColorV).toColor();
      _darkColor =
          HSVColor.fromAHSV(1.0, darkColorH, darkColorS, darkColorV).toColor();
  ...
  Widget build(BuildContext context) {
    ...
    final radialRotation = _lightYearMode
        ? (DateTime.now().second * 1000.0 + DateTime.now().millisecond) *
                radiansPerMilliSeconds -
            radians(90)
        : (_now.second * 1000.0 + _now.millisecond) * radiansPerMilliSeconds -
            radians(90);
          ...
          builder: (context, constraints) {
            return Container(
              decoration: BoxDecoration(
                ...
                colors: [
                  _lightColor.withAlpha(255),
                  _midColor.withAlpha(255),
                  _darkColor.withAlpha(255)
                ],
